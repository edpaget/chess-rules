...
module Chess
  class Piece
    ...
    def squares_between(square)
      ...
      if s_file == e_file
        ...
        count_alpha, finish_alpha, count, finish = if s_file < e_file
          [s_file, e_file, s_rank,
           e_rank,]
        else
          [e_file, s_file, e_rank, s_rank]
        end
        count_alpha += 1
        if count > finish
          count -= 1
  ...
  class Knight < Piece
    ...
    def move?(square)
      s_file, s_rank = rank_and_file(@square)
      e_file, e_rank = rank_and_file(square)
      ((s_file - e_file).abs == 2 && (s_rank - e_rank).abs == 1) ||
        ((s_file - e_file).abs == 1 && (s_rank - e_rank).abs == 2)
    end
  end
